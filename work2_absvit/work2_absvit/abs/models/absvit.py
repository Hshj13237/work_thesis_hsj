...
class Decode_Block(nn.Module):
    ...
    def forward(self, x, td=None):
        x = self.linear(x)
        out = x + self.ls1(self.attn(self.norm1(x), td))
        # x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.decoders = nn.Sequential(*[
            Decode_Block(
                dim=embed_dim, num_heads=3, mlp_ratio=mlp_ratio, qkv_bias=False, init_values=init_values,
        ...
        # add
        self.linearL = nn.Linear(self.num_classes, self.embed_dim, bias=False)
        self.linearL.weight = torch.nn.parameter.Parameter(
            torch.randn(self.embed_dim).unsqueeze(-1).repeat(1, self.num_classes), requires_grad=True)
    ...
    def forward(self, x, return_all_features=False, targets=None):
        input = x
        output_each_iter = []
        idx_each_iter = [torch.arange(x.shape[0], device=x.device), torch.arange(x.shape[0], device=x.device)]
        ...
        if targets is not None:
            promptL = self.linearL(targets).unsqueeze(dim=-1)
            prompt = F.normalize(promptL, dim=1) * 0.01 + F.normalize(self.prompt[None, ..., None], dim=1) * 0.99
        else:
            prompt = F.normalize(self.prompt[None, ..., None], dim=1)
        cos_sim = F.normalize(x, dim=-1) @ prompt  # B, N, 1
        ...
        if return_all_features:
            return output_each_iter, var_loss
        else:
            return x
...
@register_model
def absvit_small_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def absvit_base_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(
