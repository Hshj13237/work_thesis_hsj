...
@Author  :   Hong Shijie
@Version :   8
@Note    :   在v6.11的基础上，让原始图像参与y神经元激活值计算
'''
...
class DN(nn.Module):
    def __init__(self, img_dim, input_dim, y_neuron_num, y_top_k, y_bottom_up_percent, y_top_down_percent, y_img_percent, z_neuron_num, synapse_flag):
        super(DN, self).__init__()
        self.x_neuron_num = input_dim[0] * input_dim[1]
        self.img_neuron_num = img_dim[0] * img_dim[1]
        ...
        self.y_top_down_percent = y_top_down_percent
        self.y_img_percent = y_img_percent
        self.synapse_age = 1.0
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

        self.x2y = nn.Linear(self.x_neuron_num, self.y_neuron_num, bias=False)
        self.img2y = nn.Linear(self.img_neuron_num, self.y_neuron_num, bias=False)
        ...
        self.img2y.weight.data = torch.rand(self.img2y.weight.data.shape)
    ...
    def forward(self, img, x, z, mode, per_item, global_step):
        batch = int(x.shape[0])
        x = x.view(batch, -1)
        ...
        if mode == 'train':
            with torch.no_grad():
                for num in range(per_item):
                    ...
                    self.img2y.weight.data = F.normalize(self.img2y.weight.data, dim=1)

                    y_bottom_up_response = self.x2y(x)
                    y_top_down_response = self.z2y(z_hot)
                    y_img_response = self.img2y(img)
                    y_pre_response = self.y_bottom_up_percent * y_bottom_up_response + \
                        self.y_top_down_percent * y_top_down_response + self.y_img_percent * y_img_response
                    max_response, max_index, lower_index = self.top_k_competition(y_pre_response, 1)
                    self.hebbian_learning(img, x, z_hot, max_index, max_response, batch, per_item, num, lower_index, global_step)
            ...
            with torch.no_grad():
                ...
                y_bottom_up_response = self.x2y(x)
                y_img_response = self.img2y(img)
                y_up_response = (self.y_bottom_up_percent * y_bottom_up_response +
                                 self.y_img_percent * y_img_response) / (self.y_bottom_up_percent + self.y_img_percent)
                y_activated_num = torch.where(self.y_neuron_age >= 1, 1, 0)
                y_activated_num = y_activated_num.to(self.device)
                y_pre_response = y_up_response.mul(y_activated_num)
    ...
    def hebbian_learning(self, img, x, z_hot, max_index, max_response, batch, per_item, num, lower_index, global_step):
        ...
        self.weight_function(x, 'x2y', y_lr, batch, divisor)
        self.weight_function(z_hot, 'z2y', y_lr, batch, divisor)
        self.weight_function(img, 'img2y', y_lr, batch, divisor)
        ...
        if num == per_item - 1:
            ...
            # z_lr = torch.where(z_lr >= 1 / (self.z_neuron_age + 1.0), z_lr, 1 / (self.z_neuron_age + 1.0))
            # z_lr = torch.where(self.z_neuron_age > 10, self.z_lr_limit, 1 / (self.z_neuron_age + 1.0))
    ...
    def reset(self, new_num):
        ...
        temp_data = self.img2y.weight.data
        self.img2y = nn.Linear(self.img_neuron_num, self.y_neuron_num, bias=False)
        self.img2y.weight.data = torch.rand(self.img2y.weight.data.shape)
        # self.x2y.weight.data = F.normalize(self.x2y.weight.data, dim=1)
        # self.x2y.weight.data[temp_data.size()[0]:-1, :] = temp_data
        # self.x2y.weight.data[temp_data.size()[0]:-1, :] = temp_data
        self.img2y.weight[:temp_data.size()[0]].data.copy_(temp_data)
    ...
    def weight_function(self, inputs, mode, y_lr, batch, divisor):
        if mode == 'x2y':
            temp_weight = ((1 - y_lr.repeat(1, 1, self.x2y.weight.data.shape[1])).mul(
                self.x2y.weight.data.unsqueeze(0)) + y_lr.repeat(1, 1, self.x2y.weight.data.shape[1]).mul(
                inputs.unsqueeze(1))) - self.x2y.weight.data.unsqueeze(0).repeat(batch, 1, 1)
            temp_weight = (torch.sum(temp_weight, dim=0)).div(divisor.unsqueeze(1))

            self.x2y.weight.data = self.x2y.weight.data + temp_weight

        elif mode == 'z2y':
            temp_weight = ((1 - y_lr.repeat(1, 1, self.z2y.weight.data.shape[1])).mul(
                self.z2y.weight.data.unsqueeze(0)) +
                           y_lr.repeat(1, 1, self.z2y.weight.data.shape[1]).mul(inputs.unsqueeze(1))) - \
            ...
            temp_weight_all = torch.zeros(self.img2y.weight.size()).cuda()
            batch_cnt = 0
            while batch_cnt < batch:
                temp_weight = ((1 - y_lr[batch_cnt:batch_cnt+self.batch_batch].repeat(
                    1, 1, self.img2y.weight.data.shape[1])).mul(self.img2y.weight.data.unsqueeze(0)) +
                               y_lr[batch_cnt:batch_cnt+self.batch_batch].repeat(
                                   1, 1, self.img2y.weight.data.shape[1]).mul(
                                   inputs[batch_cnt:batch_cnt+self.batch_batch].unsqueeze(1))) - \
                    self.img2y.weight.data.unsqueeze(0).repeat(self.batch_batch, 1, 1)
                temp_weight_all += torch.sum(temp_weight, dim=0)

                batch_cnt += self.batch_batch

            temp_weight_all = temp_weight_all.div(divisor.unsqueeze(1))

            self.img2y.weight.data = self.img2y.weight.data + temp_weight_all
