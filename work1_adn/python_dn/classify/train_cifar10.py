'''
@File    :   train_cifar10_v2.py
@Time    :   2023/04/04 10:00
@Author  :   Hong Shijie
@Version :   2.0
@Note    :   适配于resnet和dn结合的工作，配套cnn_dn_v1.py与dn_torch_v4.py
'''
import os
...
def get_dir(saver_dir):
    cnt = 0
    while True:
        saver_f = os.path.join(saver_dir, 'cifar10_v2_')
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer_b, step_size=15, gamma=0.5)
...
for epo in range(epochs):
    ...
    for img, label in loop_train:
        ...
        loop_train.set_postfix(loss=loss.item(), lr=scheduler.get_last_lr()[0],
                               activated_number=activated_num.item(), memory='{}MB'.format(int(torch.cuda.memory_allocated() / 1024 / 1024)))
