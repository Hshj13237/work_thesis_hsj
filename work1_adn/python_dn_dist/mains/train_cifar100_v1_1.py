...
def main():
    ...
    for epo in range(epochs):
        ...
        model.dn.y_neuron_age.data = torch.zeros((1, model.dn.y_neuron_num)).to('cuda:0')
        model.dn.y_threshold.data = torch.zeros((1, model.dn.y_neuron_num)).to('cuda:0')
        model.dn.z_neuron_age.data = torch.zeros((1, model.dn.z_neuron_num)).to('cuda:0')

        model.dn.y2z.weight.data = torch.zeros(model.dn.y2z.weight.data.shape).to('cuda:0')

        for _ in range(1):
            unrevised_activated_num = train(train_loader, model, optimizer_b, scheduler, optimizer_d, epo, "lock_backbone")
        ...
        for _ in range(2):
            unrevised_activated_num = train(train_loader, model, optimizer_b, scheduler, optimizer_d, epo, "lock_dn")

            tra_acc = test(train_loader, model, 'train', epo)
            acc = test(test_loader, model, 'valid', epo)

            save_excel('联合bp', save_dir, epo, 0,
                       unrevised_num=unrevised_activated_num,
        ...
        # model.dn.x2y.weight.data = torch.rand(model.dn.x2y.weight.data.shape).to('cuda:0')
        # model.dn.y2z.weight.data = torch.rand(model.dn.y2z.weight.data.shape).to('cuda:0')
        # model.dn.y_neuron_age.data = torch.zeros(model.dn.y_neuron_age.data.shape).to('cuda:0')
        # model.dn.z_neuron_age.data = torch.zeros(model.dn.z_neuron_age.data.shape).to('cuda:0')
        # model.dn.y_threshold.data = torch.zeros(model.dn.y_threshold.data.shape).to('cuda:0')
...
def train(train_loader, model, optimizer_b, scheduler, optimizer_d, epo, mode):
    ...
    if mode == "lock_backbone":
        for img, label in loop_train:
            ...
            loop_train.set_postfix(lr=scheduler.get_last_lr()[0], activated_number=activated_num.item(),
        ...
        for img, label in loop_train:
            ...
            for cnt in range(backbone_times):
                ...
                loss = criterion(pre_output, label.to('cuda:0'))
