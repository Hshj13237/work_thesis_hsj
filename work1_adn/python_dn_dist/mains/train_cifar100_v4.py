...
from utils.sets.cifar100_dataset import Cifar100Dataset
import random
...
def main():
    ...
    train_set = torchvision.datasets.CIFAR100('../data/cifar100', train=True, download=True,
    ...
    # train_set.data, train_set.targets = DatasetPro(train_set.data, train_set.targets)
    # train_set = Cifar100Dataset(train_set.data, train_set.targets, train_set.transform)
    ...
    # for i in ['dn.y_neuron_age', 'dn.y_threshold', 'dn.x2y.weight', 'dn.z2y.weight', 'dn.y2z.weight', 'dn.z_neuron_age',
    #           'dn.x2y2.weight', 'dn.x2y3.weight', 'dn.x2y4.weight']:
    for i in ['dn.y_neuron_age', 'dn.y_threshold', 'dn.x2y.weight', 'dn.z2y.weight', 'dn.y2z.weight', 'dn.z_neuron_age']:
        state_load.pop(i)
    model.load_state_dict(state_load, strict=False)


    # state_load = torch.load('../results/best_e14_acc7887.pth')
    # y_neuron_num = state_load['dn.y_neuron_age'].size()[1]
    # model = concat_net(batch_size, dn_input_dim, y_neuron_num, y_top_k, y_bottom_up_percent, z_neuron_num, synapse_flag, dn_input_dim2)
    # model.load_state_dict(state_load, strict=False)

    optimizer_b = torch.optim.AdamW(nn.ModuleList([model.backbone, model.fc]).parameters(), lr=learning_rate)
    # optimizer_b = torch.optim.SGD(nn.ModuleList([model.backbone, model.fc]).parameters(), lr=learning_rate)

    scheduler = torch.optim.lr_scheduler.StepLR(optimizer_b, step_size=100, gamma=0.9)

    optimizer_d = torch.optim.AdamW(model.dn.parameters(), lr=learning_rate)
    ...
    for epo in range(epochs):
        ...
        if (epo % 50 == 0):
            torch.save(model.state_dict(), os.path.join(save_dir, 'model_e{}_acc{}.pth'.format(epo, int(acc * 10000))))
        # scheduler.step()
        ...
        if acc >= max_acc:
            torch.save(model.state_dict(), os.path.join(save_dir, 'best_e{}_acc{}.pth'.format(epo, int(acc * 10000))))
            max_acc = acc
...
def train(train_loader, model, optimizer_b, scheduler, optimizer_d, epo, mode):
    ...
    for img, label in loop_train:
        ...
        if mode == "lock_backbone":
            ...
            if (model.dn.y_neuron_age.size()[1] - activated_num) < batch_size:
                y_neuron_num = model.dn.y_neuron_age.size()[1] + batch_size
...
def remove(model_dn, epo, save_dir):
    ...
    # model_dn.dn.x2y2.weight.data[temp_y_indices.squeeze(0).numpy(), :] = torch.rand((
    #     len(temp_y_indices.squeeze(0)), model_dn.dn.x2y2.weight.size()[1])).cuda()
    # model_dn.dn.x2y3.weight.data[temp_y_indices.squeeze(0).numpy(), :] = torch.rand((
    #     len(temp_y_indices.squeeze(0)), model_dn.dn.x2y3.weight.size()[1])).cuda()
    # model_dn.dn.x2y4.weight.data[temp_y_indices.squeeze(0).numpy(), :] = torch.rand((
    #     len(temp_y_indices.squeeze(0)), model_dn.dn.x2y4.weight.size()[1])).cuda()
    #
    # if epo > 0:
...
def test(test_loader, model, name, epo):
    ...
    with torch.no_grad():
        for img, label in loop_test:
            ...
            loop_test.set_postfix(acc=num_correct.data.cpu().numpy(), memory='{}MB'.format(int(torch.cuda.memory_allocated() / 1024 / 1024)))
...
def DatasetPro(data, targets):
    targets = np.array(targets)
    idxs = targets.argsort()[40000:50000]
    targets = np.array(targets[idxs]).tolist()
    data = data[idxs]
    return data, targets
