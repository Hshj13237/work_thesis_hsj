...
class concat_net(nn.Module):
    def __init__(self, batch_size, dn_input_dim, y_neuron_num, y_top_k, y_bottom_up_percent, z_neuron_num, synapse_flag):
        ...
        self.dn2 = DN([32, 32], y_neuron_num, y_top_k, y_bottom_up_percent, z_neuron_num, synapse_flag).to('cuda:0')
    ...
    def forward(self, x, z, mode, per_item, epo):
        ...
        if mode == 'lock_backbone':
            ...
            y_activated_num2 = self.dn2(img.to('cuda:0'), z.to('cuda:0'), mode, per_item, epo)
            return y_activated_num, y_activated_num2
        elif mode == 'lock_dn':
        ...
        #     return output
            ...
            output_dn2 = self.dn2(img.to('cuda:0'), z.to('cuda:0'), mode, 1, epo)
            output_fc = self.fc(x)

            output_dn = torch.where(output_dn >= output_dn2, output_dn, output_dn2)
