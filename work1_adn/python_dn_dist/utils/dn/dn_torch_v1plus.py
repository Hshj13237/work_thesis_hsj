'''
@File    :   dn_torch_v6_8_2.py
@Time    :   2023/06/09 16:00
@Author  :   Hong Shijie
@Version :   6.8.2
@Note    :   在v6.8的基础上，结合v6.11的内容，使其适用于联合bp方案
'''
...
class DN(nn.Module):
        ...
        if mode == 'lock_backbone':
            with torch.no_grad():
                for num in range(per_item):
                    ...
                    self.hebbian_learning(x, z_hot, max_index, max_response, batch, per_item, lower_index, num)
            ...
            var_loss = F.mse_loss(self.x2y.weight.data[torch.argmax(y, dim=1), :], x)
    ...
    def hebbian_learning(self, x, z_hot, max_index, max_response, batch, per_item, lower_index, num):
