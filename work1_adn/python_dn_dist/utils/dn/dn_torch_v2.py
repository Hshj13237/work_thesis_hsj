...
class DN(nn.Module):
    def __init__(self, input_dim, y_neuron_num, y_top_k, y_bottom_up_percent, z_neuron_num, synapse_flag, input_dim_2):
        ...
        self.x2y2 = nn.Linear(self.x_neuron_num_2, self.y_neuron_num, bias=False)
    ...
    def forward(self, x, z, mode, per_item, epo, img):
        batch = int(x.shape[0])
        x = x.view(batch, -1)
        ...
        if mode == 'lock_backbone':
            with torch.no_grad():
                for num in range(per_item):
                    ...
                    # y_bottom_up_response_2 = self.x2y2(img)
                    ...
                    y_pre_response = 0.4 * y_bottom_up_response + 0.6 * y_top_down_response

                    max_response, max_index, lower_index = self.top_k_competition(y_pre_response, mode, z)
                    self.hebbian_learning(x, z_hot, max_index, max_response, batch, per_item, lower_index, num, img)
            ...
            y_bottom_up_response = 0.1 * self.x2y(x) + 0.9 * self.x2y2(img)
    ...
    def top_k_competition(self, y_pre_response, mode, z):
        if mode == 'lock_backbone':
            ...
            add_index = torch.where(judge + cor_judge < 1)
    ...
    def hebbian_learning(self, x, z_hot, max_index, max_response, batch, per_item, lower_index, num, img):
        ...
        for i in range(int(batch / self.mini_batch * 4)):
            temp_weight = ((1 - y_lr[i * int(self.mini_batch/4):(i+1) * int(self.mini_batch / 4)].repeat(1, 1, self.x2y2.weight.data.shape[1])).mul(self.x2y2.weight.data.unsqueeze(0)) +
                           y_lr[i * int(self.mini_batch/4):(i+1) * int(self.mini_batch / 4)].repeat(1, 1, self.x2y2.weight.data.shape[1]).mul(img[i * int(self.mini_batch/4):(i+1) * int(self.mini_batch / 4)].unsqueeze(1))) - \
