...
@Author  :   Hong Shijie
@Version :   3.5
@related :   cnn_dn_v1_2 backbone_v1 dn_torch_v6_10/v6_11
@Note    :   在v3.4的基础上,使数据可以写入excel中
'''
import os
    ...
    if state == 'dn':
        with torch.no_grad():
            for img, label in loop_train:
                ...
                _, min_index = torch.min(output_fc, dim=1)
                z = torch.cat((label.unsqueeze(0), min_index.unsqueeze(0)), dim=0)
                activated_num = model_dn(img, z, 'train', dn_items, global_step)
