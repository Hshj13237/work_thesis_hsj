'''
@File    :   train_cifar100_v1_2.py
@Time    :   2023/06/20 12:30
@Author  :   Hong Shijie
@Version :   1.2
@related :   cnn_dn_v1_2_1 backbone_v1 dn_torch_v8
@Note    :   在v1.2的基础上,引入原始图像参与y神经元激活值计算方案
'''
import os
...
from utils.concat.cnn_dn_v1_2_1 import concat_net
...
y_top_k = 1
y_bottom_up_percent, y_top_down_percent, y_img_percent = 0.35, 0.4, 0.25
z_neuron_num = 100
synapse_flag = False
dn_items = 1
learning_rate = 0.001
...
def main():
    ...
    model_dn = concat_net(batch_size, img_dim, dn_input_dim, y_neuron_num, y_top_k, y_bottom_up_percent,
    ...
    state_load_1 = torch.load('../results/model_e246_acc726.pth')
    state_load_1.pop('resnet18.fc.weight')
    state_load_1.pop('resnet18.fc.bias')
    model_dn.backbone.load_state_dict(state_load_1)
...
def losses(output_fc, output_dn, label):
    ...
    flag = torch.nonzero(fc_correct == True).squeeze(1).cpu().numpy()
