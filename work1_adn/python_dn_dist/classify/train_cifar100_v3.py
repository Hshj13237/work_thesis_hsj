'''
@File    :   train_cifar100_v3.py
@Time    :   2023/06/05 21:00
@Author  :   Hong Shijie
@Version :   3
@related :   cnn_dn_v2/v2_1 backbone_v2_1 dn_torch_v7
@Note    :   在train_cifar10_v4的基础上，适应于cifar100数据集
'''
import os
...
from utils.concat.cnn_dn_v2 import concat_net
...
def main():
    ...
    for epo in range(epochs):
        ...
        tra_fc_acc, tra_dn_acc, tra_acc = test(train_loader, model, 'train')
        fc_acc, dn_acc, acc = test(test_loader, model, 'valid')
        ...
        save_excel('mul', save_dir, epo, 0,
                   ...
                   train_fc_acc=tra_fc_acc, train_dn_acc=tra_dn_acc, train_acc=tra_acc,
                   valid_fc_acc=fc_acc, valid_dn_acc=dn_acc, valid_acc=acc)
...
def test(test_loader, model, name):
    ...
    with torch.no_grad():
        for img, label in loop_test:
            ...
            output_fc, output_dn, output = model(img, label, 'test', dn_items)
            ...
            loop_test.set_postfix(acc=acc, fc=fc_acc, dn=dn_acc, memory='{}MB'.format(int(torch.cuda.memory_allocated() / 1024 / 1024)))
    return fc_acc, dn_acc, acc
