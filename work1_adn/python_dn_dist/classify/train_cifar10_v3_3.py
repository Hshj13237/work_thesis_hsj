'''
@File    :   train_cifar10_v3_3.py
@Time    :   2023/05/28 18:00
@Author  :   Hong Shijie
@Version :   3.3
@related :   cnn_dn_v1 backbone_v1 dn_torch_v6_9
@Note    :   在v3.2的基础上,适用于教师学生模型中循环锁定方案,
'''
import os
...
from utils.concat.cnn_dn_v1_1 import concat_net
...
def main():
    ...
    for epo in range(epochs):
        train(train_loader, model_dn, model_fc, optimizer, scheduler, epo, 'dn', None)
...
def train(train_loader, model_dn, model_fc, optimizer, scheduler, epo, state, backbone_cnt):
    ...
    if state == 'dn':
        for img, label in loop_train:
            ...
            activated_num = model_dn(img, label, 'train', dn_items)
        ...
        for img, label in loop_train:
            ...
            output_dn = model_dn(img, label, 'test', dn_items)
