'''
@File    :   train_cifar10_v1_2.py
@Time    :   2023/04/23 19:00
@Author  :   Hong Shijie
@Version :   1.2
@Note    :   在v1.1的基础上使其适用于dn_torch_v5.1的min处理方式
'''
import torch.utils.data
import torchvision
import utils.dn.dn_torch_v5_1 as DN
...
for epo in range(epochs):
    with torch.no_grad():
        ...
        for img, label in loop_train:
            ...
            activated_num, max_response = dn(img, label, 'train', 2)
            with open('./res.txt', 'a') as f:
                f.write('{}\n'.format(max_response.cpu().numpy()))
