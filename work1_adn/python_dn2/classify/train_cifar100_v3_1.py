...
def main():
    ...
    optimizer = torch.optim.SGD(model.backbone.parameters(), lr=learning_rate)
    ...
    for epo in range(epochs):
        ...
        tra_dn_acc0, tra_acc0 = test(train_loader, model, 'train', 0)
        dn_acc0, acc0 = test(test_loader, model, 'valid', 0)

        tra_dn_acc1, tra_acc1 = test(train_loader, model, 'train', 1)
        dn_acc1, acc1 = test(test_loader, model, 'valid', 1)

        tra_dn_acc2, tra_acc2 = test(train_loader, model, 'train', 2)
        dn_acc2, acc2 = test(test_loader, model, 'valid', 2)

        save_excel('mul', save_dir, epo, 0,
                   unrevised_num=unrevised_activated_num, revised_num=revised_activated_num,
                   train_dn_acc0=tra_dn_acc0, train_acc0=tra_acc0,
                   valid_dn_acc0=dn_acc0, valid_acc0=acc0,
                   train_dn_acc1=tra_dn_acc1, train_acc1=tra_acc1,
                   valid_dn_acc1=dn_acc1, valid_acc1=acc1,
                   train_dn_acc2=tra_dn_acc2, train_acc2=tra_acc2,
                   valid_dn_acc2=dn_acc2, valid_acc2=acc2
                   )
...
def train(train_loader, model, optimizer, scheduler, epo):
    ...
    for img, label in loop_train:
        ...
        output, activated_num = model(img, label, 'train', dn_items, global_step, None)
...
def test(test_loader, model, name, test_cnt):
    ...
    with torch.no_grad():
        for img, label in loop_test:
            ...
            output_dn, output = model(img, label, 'test', dn_items, global_step, test_cnt)
