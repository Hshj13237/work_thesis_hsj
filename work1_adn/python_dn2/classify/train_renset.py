...
def get_dir(saver_dir):
    cnt = 0
    while True:
        saver_f = os.path.join(saver_dir, 'resnet18_v2_')
...
backbone = backbone(z_neuron_num, batch_size)
...
for epo in range(epochs):
    ...
    for img, label in loop_train:
        img, label = img.to(backbone.device), label.to(backbone.device)
    ...
    loop_test = tqdm(train_loader, ncols=80)
    num_correct = 0
    total_loss = 0
    with torch.no_grad():
