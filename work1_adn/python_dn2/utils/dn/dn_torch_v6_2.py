'''
@File    :   dn_torch_v6_2.py
@Time    :   2023/05/03 14:00
@Author  :   Hong Shijie
@Version :   6.2
@Note    :   在v6.1的基础上，学习率与单次DN循环数量有关
'''
...
class DN(nn.Module):
        ...
        if mode == 'train':
            with torch.no_grad():
                for num in range(per_item):
                    ...
                    self.hebbian_learning(x, z_hot, max_index, max_response, batch, per_item)
    ...
    def hebbian_learning(self, x, z_hot, max_index, max_response, batch, per_item):
        ...
        "单个batch最多增加1"
        new_age.index_put_(indices=tuple(idxs.unsqueeze(0)), values=((torch.ones(cnts.size()).to(self.device)) / per_item))
        ...
        cnts = torch.where(cnts > 0, 1.0 / per_item, 0)
