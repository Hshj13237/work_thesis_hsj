'''
@File    :   dn_torch_v6_12.py
@Time    :   2023/07/25 16:00
@Author  :   Hong Shijie
@Version :   6.12
@Note    :   在v6.11的基础上，加入侧向抑制
'''
...
class DN(nn.Module):
    def __init__(self, input_dim, y_neuron_num, y_top_k, y_bottom_up_percent, z_neuron_num, synapse_flag):
        ...
        self.y2y = nn.Linear(self.y_neuron_num, self.y_neuron_num, bias=False)
        ...
        self.y2y.weight.data = torch.zeros(self.y2y.weight.data.shape)
        ...
        if mode == 'train':
            with torch.no_grad():
                for num in range(per_item):
                    ...
                    y_pre_lateral_response = self.y_bottom_up_percent * y_bottom_up_response + self.y_top_down_percent * y_top_down_response
                    y_lateral_response = self.y2y(y_pre_lateral_response)
                    y_pre_response = (self.y_lateral_percent * y_lateral_response + y_pre_lateral_response) / (
                        self.y_bottom_up_percent + self.y_top_down_percent + self.y_lateral_percent
                    )
                    max_response, max_index, lower_index = self.top_k_competition(y_pre_response, 1)
                    self.hebbian_learning(x, z_hot, max_index, max_response, batch, per_item, num, lower_index, global_step)
            ...
            with torch.no_grad():
                ...
                y_bottom_up_response = self.x2y(x)
                y_lateral_response = self.y2y(y_bottom_up_response)
                y_pre_response = (self.y_bottom_up_percent * y_bottom_up_response + self.y_lateral_percent * y_lateral_response) / (
                    self.y_bottom_up_percent + self.y_lateral_percent
                )

                y_activated_num = torch.where(self.y_neuron_age >= 1, 1, 0)
                y_activated_num = y_activated_num.to(self.device)
                y_pre_response = y_pre_response.mul(y_activated_num)
    ...
    def hebbian_learning(self, x, z_hot, max_index, max_response, batch, per_item, num, lower_index, global_step):
        ...
        # y to y
        if self.y_neuron_age.size()[1] <= 5000:
            piece = 8
        elif self.y_neuron_age.size()[1] <= 10000:
            piece = 4
        elif self.y_neuron_age.size()[1] <= 15000:
            piece = 2
        else:
            piece = 1
        for i in range(int(batch / piece)):
            temp_y_lr = y_lr[i * piece:(i+1) * piece]
            temp_y_response = y_response[i * piece:(i+1) * piece]
            temp_weight = ((1 - temp_y_lr.repeat(1, 1, self.y2y.weight.data.shape[1])).mul(self.y2y.weight.data.unsqueeze(0)) +
                           temp_y_lr.repeat(1, 1, self.y2y.weight.data.shape[1]).mul(temp_y_response.unsqueeze(1))) - \
                self.y2y.weight.data.unsqueeze(0).repeat(piece, 1, 1)
            temp_weight = (torch.sum(temp_weight, dim=0)) / piece

            self.y2y.weight.data = self.y2y.weight.data + temp_weight
    ...
    def reset(self, new_num):
        ...
        temp_data = self.y2y.weight.data
        self.y2y = nn.Linear(self.y_neuron_num, self.y_neuron_num, bias=False)
        self.y2y.weight.data = torch.zeros(self.y2y.weight.data.shape)
        self.y2y.weight[:temp_data.size()[0], :temp_data.size()[1]].data.copy_(temp_data)
