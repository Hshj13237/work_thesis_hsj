'''
@File    :   dn_torch_v6_10.py
@Time    :   2023/05/28 19:00
@Author  :   Hong Shijie
@Version :   6.10
@Note    :   在v6.9的基础上，使z神经元的学习率与z神经元个数有关
'''
...
class DN(nn.Module):
        ...
        if mode == 'train':
            with torch.no_grad():
                for num in range(per_item):
                    ...
                    self.hebbian_learning(x, z_hot, max_index, max_response, batch, per_item, num, global_step)
    ...
    def hebbian_learning(self, x, z_hot, max_index, max_response, batch, per_item, num, global_step):
