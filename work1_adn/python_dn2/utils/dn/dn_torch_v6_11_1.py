'''
@File    :   dn_torch_v6_11_1.py
@Time    :   2023/7/25 16:00
@Author  :   Hong Shijie
@Version :   6.11.1
@Note    :   在v6.11的基础上，对x->y权重更新进行分批次处理
'''
...
class DN(nn.Module):
    def __init__(self, input_dim, y_neuron_num, y_top_k, y_bottom_up_percent, z_neuron_num, synapse_flag):
        ...
        self.x_neuron_num = input_dim[0] * input_dim[1] * input_dim[2]
    ...
    def hebbian_learning(self, x, z_hot, max_index, max_response, batch, per_item, num, lower_index, global_step):
        ...
        # x to y
        if self.y_neuron_age.size()[1] <= 10000:
            piece = 16
        elif self.y_neuron_age.size()[1] <= 20000:
            piece = 8
        elif self.y_neuron_age.size()[1] <= 25000:
            piece = 4
        elif self.y_neuron_age.size()[1] <= 30000:
            piece = 2
        else:
            piece = 1
        temp_weight = torch.zeros(self.x2y.weight.data.size()).to(self.device)
        for i in range(int(batch / piece)):
            temp_y_lr = y_lr[i * piece:(i + 1) * piece]
            temp_x = x[i * piece:(i + 1) * piece]
            temp_weight_piece = ((1 - temp_y_lr.repeat(1, 1, self.x2y.weight.data.shape[1])).mul(self.x2y.weight.data.unsqueeze(0)) +
                                 temp_y_lr.repeat(1, 1, self.x2y.weight.data.shape[1]).mul(temp_x.unsqueeze(1))) - \
                self.x2y.weight.data.unsqueeze(0).repeat(piece, 1, 1)
            temp_weight += torch.sum(temp_weight_piece, dim=0)

        self.x2y.weight.data = self.x2y.weight.data + temp_weight.div(divisor.unsqueeze(1))
